class Services::CreateCategoryContact
  def self.call

    CategoryContact.all.each {|c| c.update(parsing: false)}

    url_source = "https://contactplus.ru/catalog"
    selector_top_level = '.item-catalog .sub-small-prw a'
    selector_other_level = '#container .isotope .small-result .product_item_prw_text .product_item_prw_title a'
  ...
  def self.create_structure(data_category_from_up, selector_top_level, selector_other_level, current_top_level = false)
    ...
    url = "https://contactplus.ru#{url}" unless url[/http|https/]
    ...
    p category = CategoryContact.find_by(category_path: data_category_from_up[:category_path])
    unless category
      p category = CategoryContact.create!(
  ...
  def self.delete_not_exits_subcategory(old_sub, new_sub)
    ...
    (old_sub - new_sub).each do |category_path|
      CategoryContact.find_by(category_path: category_path).destroy
    end
  end
