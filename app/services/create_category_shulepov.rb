class Services::CreateCategoryShulepov
  def self.call

    CategoryShulepov.all.each {|c| c.update(parsing: false)}

    url_source = "https://shulepov.ru/catalog"
    selector_top_level = 'nav .bxr-top-menu > li > a'
    selector_other_level = '.bx_sitemap .bx_sitemap_ul > li > .bx_sitemap_li_title a'
  ...
  def self.create_structure(data_category_from_up, selector_top_level, selector_other_level, current_top_level = false)
    ...
    url = "https://shulepov.ru#{url}" unless url[/http|https/]
    ...
    p category = CategoryShulepov.find_by(category_path: data_category_from_up[:category_path])
    unless category
      p category = CategoryShulepov.create!(
  ...
  def self.delete_not_exits_subcategory(old_sub, new_sub)
    ...
    (old_sub - new_sub).each do |category_path|
      CategoryShulepov.find_by(category_path: category_path).destroy
    end
  end
